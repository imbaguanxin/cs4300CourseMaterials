...
#include "RTScenegraphRenderer.h"
#include "INode.h"
 ...
 */
  ...
  protected:
    ...
    RTScenegraphRenderer *raytracer;
  ...
  public:
    ...
    /**
     * Sets the ray trace renderer
     * This function must be called when the scene graph is complete, otherwise not all of its
     * meshes will be known to the renderer
     * \param renderer The RTScenegraphRenderer object that will act as its renderer
     * \throws Exception
     */
    void setRaytracer(RTScenegraphRenderer *renderer) throw(runtime_error)
    {
      this->raytracer = renderer;
      ...
      for (map<string,string>::iterator it=textures.begin();
           ...
           it++)
        {
          this->raytracer->addTexture(it->first,it->second);
        }
    ...
    void raytrace(stack<glm::mat4>& modelView)
    {
      if ((root!=NULL) && (raytracer!=NULL))
        {
          raytracer->draw(root,modelView);
        }
    }
